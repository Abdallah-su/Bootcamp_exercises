...
SELECT c.first_name FROM customer c JOIN customer_profile cp ON c.id =cp.customer_id WHERE cp.isLoggedin =true;
SELECT c.first_name FROM customer_profile cp INNER JOIN customer c ON cp.customer_id =c.id;
