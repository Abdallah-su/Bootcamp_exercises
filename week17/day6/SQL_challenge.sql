...
SELECT SUM(salary IS NULL ) as salary_nulls, 
SUM(  department IS NULL ) AS depart_nulls,
SUM(hire_date IS NULL) AS date_nulls from employees;

UPDATE employees SET department='unknown' WHERE department is NULL;
SELECT SUM(hire_date IS NULL) AS date_nulls from employees;
